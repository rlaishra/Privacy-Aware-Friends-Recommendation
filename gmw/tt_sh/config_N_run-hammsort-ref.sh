#!/usr/bin/env bash
if [ ! $# -eq 3 ]; then
  echo "format: ./run.sh num_parties num_vector_lengths num_reference_parties"
...
MYEOF
    ...
    echo "create-circuit $circuit $((num_input - 1)) 1 2" >> $input_dir/configs${COUNTER}.txt
    echo "" >> $input_dir/configs${COUNTER}.txt
...
index=0
consumer_input="1"
while [ $index -lt $((total_input - 1)) ]; do
    consumer_input=$consumer_input" 0"
    let index=index+1
done
...
MYEOF
    ...
    echo "create-circuit $circuit $((num_input - 1)) 1 2" >> $input_dir/configc.txt
    echo "" >> $input_dir/configc.txt
