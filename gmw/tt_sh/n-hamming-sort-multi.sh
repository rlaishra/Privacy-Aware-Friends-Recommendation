...
input_dir=$MPC_PATH/"config_hamming_sorting2/gen"
...
echo " " >> $input_dir/configs${COUNTER}.txt
    ...
    if [ $COUNTER -le $num_half ];
    then
    #add address
      echo -e "$((COUNTER - 1))\t127.0.0.1\t$((port_init+COUNTER-1))" >> $input_dir/address.txt
    else #[ $COUNTER -ge $num_half ];
    #then
      echo -e "$((COUNTER - 1))\t128.230.213.57\t$((port_init+COUNTER-1))" >> $input_dir/address.txt
    fi
