...
#create fix input files (just for debug)
temp="0 1 1 0 1 0 0" #alice 	= {0, 1101, pid=00}
echo "$temp" >> $input_dir/inputs1.txt

temp="1 0 0 1 1 0 1" #bob 	= {1, 0011, pid=01}
echo "$temp" >> $input_dir/inputs2.txt

temp="1 1 1 1 1 1 0" #charlie = {1, 1111, pid=10}
echo "$temp" >> $input_dir/inputs3.txt

temp="0 0 0 0 0 1 1" #david 	= {0, 0000, pid=11}
echo "$temp" >> $input_dir/inputs4.txt
#dist(b,a)=3, dist(b,d)=2, dist(b)=min(3,2)=2
#dist(c,a)=1, dist(c,d)=4, dist(c)=min(1,4)=1
#output should be [charlie, bob], i.e., {1 0 0 1}
