...
#define RETRY_CONNECT			100000 //20
#define CONNECT_TIMEO_MILISEC	900000
...
public:
	...
	void FillRand(INT64 bits, BYTE* seed, int& pid, int& cnt)
	{
		INT64 size = (bits - 1) / SHA1_BYTES + 1;
		if( size <= m_nRand ) return;

		sha1_context sha;
		for (INT64 i = m_nRand; i<size; i++)
		{
			sha1_starts(&sha);
	...
	void Create(INT64 bits, BYTE* seed, int& pid, int& cnt)
	{
	...
	void CreateinBytes(INT64 bytes)
	{
		if( m_pBits ) delete [] m_pBits;
		m_pBits = new BYTE[bytes];
	}

	void Create(INT64 bits, sha1_context& sha0, int& cnt)
	{
		if( m_pBits ) delete [] m_pBits;
