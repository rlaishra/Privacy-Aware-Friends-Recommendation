...
void CParty::Run()
{
	...
	// socket stuff
	cout << "( " << m_nPID <<  " ) ******* Establishing Connection... *******" << endl;
	if(!EstablishConnection())
		return;

	// setup ot extension
	cout << "( " << m_nPID <<  " ) ******* Setting up OT Extension... ******* " << endl;
	if(!SetupOTExtension())
		return;
...
BOOL CParty::ThreadRunIKNPSndFirst(int nRcvID)
{
	...
	sock.Receive(pBuf, nBufSize); // for receiver at line 785
	...
	INT64 nToRcvBytes = (NUM_EXECS_NAOR_PINKAS * 4 * m_nANDGates + 7) / 8;
	...
	{
		//cout << "   "<<m_nPID << " [SEND] socket receive from " << nRcvID << endl;
		sock_rcv = min(nToRcvBytes, nWindowBytes);
		sock.Receive(pBuf2, sock_rcv);
		nToRcvBytes -= sock_rcv;
		//cout << "   "<<m_nPID << " [SEND] socket receive from " << nRcvID << " DONE! " << sock_rcv << ":" << nToRcvBytes;
		//cout << " " << nProgress << endl;
...
BOOL CParty::ThreadRunIKNPSndSecond(int nRcvID)
{
	...
	while(i<m_nANDGates)
	{
		...
		//cout << "   "<<m_nPID << " [SEND2] socket send to " << nRcvID << endl;
		sock.Send(v.GetArr(), (j*4+7)/8);
		//cout << "   "<<m_nPID << " [SEND2] socket send to " << nRcvID << " DONE!" << endl;
	}
...
BOOL CParty::ThreadRunIKNPRcvFirst(int nSndID)
{
	...
	//cout << " 	(" << nSndID << ") sent [RECV] I received PK0 " << endl;
	...
	//cout << " 		(" << nSndID << ")  [RECV] pBufIdx " << pBufIdx << endl;
	...
	//cout << " 		(" << nSndID << ")  [RECV] IMPL_PRIVIATE_CHANNEL " << endl;
#ifdef IMPL_PRIVIATE_CHANNEL
	{
		...
		//cout << "   "<<m_nPID << " [RECV] socket send to " << nSndID << " (IMPL) " << endl;
		sock.Send(pBuf, SHA1_BYTES*4);
		//cout << "   "<<m_nPID << " [RECV] socket send to " << nSndID << " DONE! (IMPL)" << endl;
	}
#endif //IMPL_PRIVIATE_CHANNEL
	//cout << " 		(" << nSndID << ")  [RECV] IMPL_PRIVIATE_CHANNEL2 " << endl;
	...
	//cout << " 		(" << nSndID << ") [RECV] m_nANDGates " << m_nANDGates << endl;
	...
	while( i < m_nANDGates )
	{
		...
		//cout << "   "<<m_nPID << " [RECV] socket send to " << nSndID << endl;
		int nSize = (j*NUM_EXECS_NAOR_PINKAS*4 + 7)/8;
 		sock.Send( v.GetArr(), nSize );
		//cout << "   "<<m_nPID << " [RECV] socket send to " << nSndID << " DONE!" << endl;
		// receive done.
...
BOOL CParty::ThreadRunIKNPRcvSecond(int nSndID)
{
	...
	while( i < m_nANDGates )
	{
		//cout << "   "<<m_nPID << " [RECV2] socket receive from " << nSndID << endl;
		sock_rcv = min(nToRcvBytes, nWindowBytes);
		sock.Receive(v.GetArr(), sock_rcv);
		//cout << "   "<<m_nPID << " [RECV2] socket receive from " << nSndID << " DONE!"<<endl;
...
void CParty::COTHelperThread::ThreadMain()
{
	...
	if( AsOTSender(nPID,m_nPartnerID )){
		//cout << " " << nPID << " SEND2 TO " << m_nPartnerID << endl;
		party->ThreadRunIKNPSndSecond(m_nPartnerID);
		//cout << " " << nPID << " SEND2 TO " << m_nPartnerID << " DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
	}
	else{
		//cout << " " << nPID << " RECEIVE2 FROM " << m_nPartnerID << endl;
		party->ThreadRunIKNPRcvSecond(m_nPartnerID);
		//cout << " " << nPID << " RECEIVE2 FROM " << m_nPartnerID << " DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
	}
